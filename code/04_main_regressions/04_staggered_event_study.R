## FxS 2026 — Staggered Event Study (Control Group 2)
## Original: 04_main_regressions/figures/02_staggered_event_study.R

rm(list = ls())
source(here::here("code/00_packages.R"))

df <- import(here("data/panel_grids_regressions.rds")) |>
  subset(year %in% 2011:2021 & sample_control_2 == 1) |>
  mutate(
    treated    = d_treated_2,
    year_conf  = ifelse(year_conf == 2022, 0, year_conf),
...
ggsave(here("output/figures/main-regressions/staggered_event_study_crops.png"), width = 7, height = 4.5)
cat("✓ Staggered event study figure saved\n")
